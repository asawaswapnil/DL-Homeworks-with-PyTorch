...
class GRU_Cell:
	...
	def forward(self, x, h):
		...
		self.Ht=self.z[24]
		self.Wzh=self.Wzh.reshape(len(self.Wzh),-1)
		self.Wrh=self.Wrh.reshape(len(self.Wrh),-1)
		self.Wh=self.Wh.reshape(len(self.Wh),-1)
		self.Wzx=self.Wzx.reshape(len(self.Wzx),-1)
		self.Wrx=self.Wrx.reshape(len(self.Wrx),-1)
		self.Wx=self.Wx.reshape(len(self.Wx),-1)
		self.x=self.x.reshape(len(self.x),-1)
		self.h=self.h.reshape(len(self.h),-1)
		return self.Ht
	...
	def backward(self, delta):
		...
		print("x",self.x)
		print("h",self.h)
		print("self.Wzh",self.Wzh)
		print("self.Wrh",self.Wrh)
		print("self.Wh",self.Wh)
		print("self.Wzx",self.Wzx)
		print("self.Wrx",self.Wrx)
		print("self.Wx",self.Wx)

		sig = Sigmoid()
		tanh=Tanh()

		self.dWzh.fill(0)
		self.dWzh=self.dWzh.reshape(len(self.dWzh),-1)
		self.dWrh.fill(0)
		self.dWrh=self.dWrh.reshape(len(self.dWrh),-1)
		self.dWh.fill(0)
		self.dWh=self.dWh.reshape(len(self.dWh),-1)
		self.dWzx.fill(0)
		self.dWzx=self.dWzx.reshape(len(self.dWzx),-1)
		self.dWrx.fill(0)
		self.dWrx=self.dWrx.reshape(len(self.dWrx),-1)
		self.dWx.fill(0)
		self.dWx=self.dWx.reshape(len(self.dWx),-1)
		self.dx.fill(0)
		self.dx=self.dx.reshape(len(self.dx),-1)
		self.dh.fill(0)
		self.dh=self.dh.reshape(len(self.dh),-1)
		...
		self.dz[21]=+delta*np.transpose(self.h)
		self.dh=+delta*np.transpose(self.z[21])
		self.dz[20]=+delta*np.transpose(self.z[11])
		self.dz[11]=+delta*np.transpose(self.z[20])
		self.dz[11]=-self.dz[21]


		self.dz[19]=+self.dz[20]*(np.transpose(1-tanh(self.z[19])*tanh(self.z[19])))
		self.dz[18]=+self.dz[19]
		self.dz[17]=+self.dz[19]
		self.dWx=+np.dot(self.x,self.dz[18])
		self.dx=+np.dot(self.dz[18],self.Wx)
		self.dWh=+np.dot(self.z[16],self.dz[17])
		self.dz[16]=+np.dot(self.dz[17],self.dWh )
		self.dz[15]=+self.dz[16]*np.transpose(self.h)
		self.dh=+self.dz[16]*np.transpose(self.z[15])

		
		self.dz[14]=+self.dz[15]*np.transpose(sig(self.z[15]))*np.transpose(1-sig(self.z[15]))
		self.dz[13]=+self.dz[14]
		self.dz[12]=+self.dz[14]
		self.dx=+np.dot(self.dz[13],self.Wrx)
		self.dWrx=+np.dot(self.x,self.dz[13])
		self.dh=+np.dot(self.dz[12],self.Wrh)
		self.dWrh=+np.dot(self.h,self.dz[12])


		self.dz[10]=+self.dz[11]*np.transpose(sig(self.z[10]))*np.transpose(1-sig(self.z[10]))
		self.dz[9]=+self.dz[10]
		self.dz[8]=+self.dz[10]
		self.dx=+np.dot(self.dz[9],self.Wzx)
		self.dWzx=+np.dot(self.x,self.dz[9])
		self.dh=+np.dot(self.dz[8],self.Wzh)
		self.dWzh=+np.dot(self.h,self.dz[8])
		for i in range(24,-1,-1):
			print(i, "z",self.z[i])
			print(i,"dz",self.dz[i])
		...
		# self.dWrx=self.dz[6]
		# self.dWx=self.dz[7]
		print("dx,dh", self.dx, self.dh)

		print("x",self.dx)
		print("h",self.dh)
		print("self.Wzh",self.dWzh)
		print("self.Wrh",self.dWrh)
		print("self.Wh",self.dWh)
		print("self.Wzx",self.dWzx)
		print("self.Wrx",self.dWrx)
		print("self.Wx",self.dWx)
