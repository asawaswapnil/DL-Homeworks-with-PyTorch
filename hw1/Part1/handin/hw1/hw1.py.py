...
class MLP(object):
    ...
    def __init__(self, input_size, output_size, hiddens, activations, weight_init_fn, bias_init_fn, criterion, lr, momentum=0.0, num_bn_layers=0):
        ...
        self.Z= np.array([np.zeros(self.ih[i]) for i in range(len(self.ih)-1)]
        self.dZ= np.array([np.zeros(self.ih[i]) for i in range(len(self.ih)-1)]
    ...
    def forward(self, x):
        ...
        self.Z[0]=x
        for i in range(1,len(nlayers)+1):
            print(self.Z[i-1].shape,W[i-1].shape)
            self.Z[i]=np.dot(Z[i-1],self.W[i-1])+self.b[i-1]
            self.Z[i]=self.activations[i].forward(self.Z[i])
        return self.Z
