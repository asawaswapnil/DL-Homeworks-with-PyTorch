...
class MLP(object):
    ...
    def __init__(self, input_size, output_size, hiddens, activations, weight_init_fn, bias_init_fn, criterion, lr, momentum=0.0, num_bn_layers=0):
        ...
        print("IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII",self.criterion)
        self.lr = lr
    ...
    def backward(self, labels):
        self.criterion.forward(self.Z[-1], labels)
        self.dLbyDy=self.criterion.derivative()
        for i in range(self.nlayers-1,-1,-1):
            print("ackprop",self.nlayers,i)
            self.dW[i]=self.activations[i].derivative()*self.dLbyDy
            self.dW[i]=np.dot(self.dW[i].T, self.Z[i]).T/self.batch_size
            self.db[i]=np.sum((self.activations[i].derivative()*self.dLbyDy),axis=0)/self.batch_size
            #self.Xt=self.x.T 
            self.dLbyDy=np.dot(self.activations[i].derivative()*self.dLbyDy,self.W[i].T)
            #print(t2.shape)
            #=(t2@t1)
            #print(t3.shape)
            #print(self.dW)
            # self.dW[0]=np.dot(self.Xt,self.dLbyDy)/self.batch_size
            # self.db[0]=np.sum(self.dLbyDy,axis=0)/self.batch_size
            #print(self.db[0])
